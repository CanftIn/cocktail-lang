#ifndef COCKTAIL_COMMON_COMMAND_LINE_H
#define COCKTAIL_COMMON_COMMAND_LINE_H
...
void ArgBuilder::MetaAction(T action) {
  COCKTAIL_CHECK(!arg_.meta_action) << "Cannot set a meta action twice!";
...
void OneOfArgBuilder::OneOfImpl(const OneOfValueT<U> (&input_values)[N],
  ...
  if ((input_values[Indices].is_default || ...)) {
    COCKTAIL_CHECK(!arg_.is_append) << "Can't append default.";
    COCKTAIL_CHECK((input_values[Indices].is_default + ... + 0) == 1)
...
#endif  // COCKTAIL_COMMON_COMMAND_LINE_H