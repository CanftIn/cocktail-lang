...
// 它基于令牌流和语言的语法，甚至没有名称查找。
//
// 这棵树旨在使深度优先遍历特别高效，
// 其中后序和反向后序（RPO，拓扑顺序）不需要额外的状态。
//
// 树的节点遵循轻量级模式，并且节点是树的句柄。
// 必须提供树本身以查询有关这些节点的信息。
//
// 节点还与从解析token流中的token一一对应。
// 每个节点都可以被视为流中特定token的树位置。
//
// 一旦建立，树就是不可变的，它旨在支持构建具有特定转换的树的高效模式。
//
// 为什么要这么写：
// 解析树是编译器和解释器中的核心数据结构。
// 它提供了一种结构化的方式来表示和处理源代码。
// Node和Tree的设计使得它们可以高效地表示和处理这种结构。
// 特别是，Node的设计使其成为一个轻量级的句柄，这意味着它
// 可以快速地复制和传递，而不需要复制整个节点的数据。
// 这对于遍历和处理树非常有用。
//
// Tree类提供了一系列的方法来构建、查询和操作解析树。
// 这些方法使得处理解析树变得更加简单和直接。
// 例如，Parse方法提供了一种从令牌缓冲区构建解析树的方法，
// 而has_errors方法则允许查询树是否 包含任何错误。
// 此外，Tree类还提供了一系列的迭代器，如PostorderIterator和
// SiblingIterator，这些迭代器使得遍历树变得更加简单和高效。
// 这是因为它们提供了一种直接的方式来访问树中的节点，
// 而不需要显式地管理遍历的状态。
class Tree : public Printable<Tree> {
 ...
 private:
  ...
  struct NodeImpl {
    ...
    //
    // 在解析树的反向后序（reverse postorder即RPO）遍历期间，
    // subtree_size是到下一个非后代节点的偏移。
    // 当此节点不是其父节点的第一个子节点（这是RPO中访问的最后一个子节点）时，
    // 这是到下一个兄弟节点的偏移。
    // 当此节点是其父节点的第一个子节点时，这将是到节点的父节点的下一个兄弟
    // 节点的偏移，
    // 或者如果父节点也是第一个子节点，则是到祖父节点的下一个兄弟节点的偏移，
    // 依此类推。
...
// 解析树的整体结构以及为什么使用 llvm::SmallVector 来存储节点数据。
// 
// 解析树通常是一种树形数据结构，其中每个节点都有一个指向其子节点的
// 指针（或索引），并且根据语法规则构建。解析树的深度优先后序遍历是
// 一种遍历方式，它按照节点的顺序访问树中的节点。这种遍历方式在分析
// 和生成代码时非常有用，因为它可以确保节点以正确的顺序访问。
// 
// llvm::SmallVector 是 LLVM 库中的一种动态数组（或向量）数据结构，
// 它在内部使用数组来存储数据。为什么使用 llvm::SmallVector 有以下好处：
// 
// 动态大小: llvm::SmallVector 具有动态大小的特性，因此它可以根据
// 需要自动扩展或缩小存储容量。这在构建解析树时很有用，因为解析树
// 的大小在解析源代码时可能会发生变化。
// 
// 低开销: llvm::SmallVector 在内部使用数组，但对于小型数据集，它
// 不会引入额外的堆内存分配，因此具有较低的开销。这对于性能至关重
// 要，特别是在解析大型源代码文件时。
// 
// 容易使用: llvm::SmallVector 提供了类似于标准库容器的接口，因此
// 它易于使用和管理。
// 
// 综合考虑以上因素，使用 llvm::SmallVector 来存储解析树节点数据是
// 一种有效的选择。它允许根据需要动态添加节点，具有较低的内存开销，
// 并提供了方便的操作接口，适用于解析器的需求。这种数据结构和遍历方
// 式可以有效地表示源代码的语法结构，并支持后续的语法分析和代码生成过程。
