#ifndef COCKTAIL_CHECK_NODE_STACK_H
#define COCKTAIL_CHECK_NODE_STACK_H
...
class NodeStack {
 public:
  ...
  auto Push(Parse::Node parse_node) -> void {
    COCKTAIL_CHECK(ParseNodeKindToIdKind(parse_tree_->node_kind(parse_node)) ==
                   IdKind::SoloParseNode)
        << "Parse kind expects an Id: " << parse_tree_->node_kind(parse_node);
    COCKTAIL_VLOG() << "Node Push " << stack_.size() << ": "
                    << parse_tree_->node_kind(parse_node) << " -> <none>\n";
    COCKTAIL_CHECK(stack_.size() < (1 << 20))
  ...
  auto Push(Parse::Node parse_node, IdT id) -> void {
    ...
    COCKTAIL_CHECK(id.is_valid()) << "Push called with invalid id: "
  ...
  auto PopWithParseNode() -> auto {
    ...
    COCKTAIL_FATAL() << "Unpoppable IdKind for parse kind: "
  ...
  auto Peek() -> auto {
    ...
    COCKTAIL_FATAL() << "Unpeekable IdKind for parse kind: "
                     << Parse::NodeKind::Create(RequiredParseKind)
                     << "; see value in ParseNodeKindToIdKind";
  }
 ...
 private:
  ...
  auto PopEntry() -> Entry {
    ...
    COCKTAIL_VLOG() << "Node Pop " << stack_.size() << ": "
  ...
  auto RequireIdKind(Parse::NodeKind parse_kind, IdKind id_kind) -> void {
    COCKTAIL_CHECK(ParseNodeKindToIdKind(parse_kind) == id_kind)
  ...
  auto RequireParseKind(Parse::Node parse_node) -> void {
    ...
    COCKTAIL_CHECK(RequiredParseKind == actual_kind)
...
#endif  // COCKTAIL_CHECK_NODE_STACK_H