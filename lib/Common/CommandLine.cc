...
namespace Cocktail::CommandLine {
...
auto operator<<(llvm::raw_ostream& output, ParseResult result)
  ...
  COCKTAIL_FATAL() << "Corrupt parse result!";
}
...
auto operator<<(llvm::raw_ostream& output, ArgKind kind) -> llvm::raw_ostream& {
  ...
  COCKTAIL_FATAL() << "Corrupt argument kind!";
}
...
auto operator<<(llvm::raw_ostream& output, CommandKind kind)
  ...
  COCKTAIL_FATAL() << "Corrupt command kind!";
}
...
void MetaPrinter::PrintVersion(const Command& command) const {
  COCKTAIL_CHECK(!command.info.version.empty())
  ...
  for (int i = 0, size = input_lines.size(); i < size;) {
    ...
    if (input_lines[i].startswith("```")) {
      ...
      do {
        ...
        ++i;
      } while (i < size && !input_lines[i].startswith(fence));
      if (i >= size) {
    ...
    if (input_lines[i].startswith("    ")) {
      ...
      do {
        out_ << indent << input_lines[i] << "\n";
        ++i;
      } while (i < size && input_lines[i].startswith("    "));
      continue;
    }
...
void MetaPrinter::PrintOptionShortName(const Arg& arg) const {
  COCKTAIL_CHECK(!arg.info.short_name.empty()) << "No short name to use.";
  out_ << "-" << arg.info.short_name;
}

void MetaPrinter::PrintArgShortValues(const Arg& arg) const {
  COCKTAIL_CHECK(arg.kind == Arg::Kind::OneOf)
...
void MetaPrinter::PrintArgHelp(const Arg& arg, llvm::StringRef indent) const {
  ...
  switch (arg.kind) {
    ...
    case Arg::Kind::Invalid:
      COCKTAIL_FATAL() << "Argument configured without any action or kind!";
  }
}
...
void Parser::SetOptionDefault(const Arg& option) {
  COCKTAIL_CHECK(option.has_default) << "No default value available!";
  switch (option.kind) {
    case Arg::Kind::Flag:
    ...
    case Arg::Kind::OneOf:
      ...
      break;
    case Arg::Kind::MetaActionOnly:
      COCKTAIL_FATAL() << "Can't set a default value for a meta action!";
    case Arg::Kind::Invalid:
      COCKTAIL_FATAL() << "Option configured without any action or kind!";
  }
}
...
auto Parser::ParseFlag(const Arg& flag, std::optional<llvm::StringRef> value)
    -> bool {
  COCKTAIL_CHECK(flag.kind == Arg::Kind::Flag)
      << "Incorrect kind: " << flag.kind;
  if (!value || *value == "true") {
...
auto Parser::ParseIntegerArgValue(const Arg& arg, llvm::StringRef value)
    -> bool {
  COCKTAIL_CHECK(arg.kind == Arg::Kind::Integer)
...
auto Parser::ParseStringArgValue(const Arg& arg, llvm::StringRef value)
    -> bool {
  COCKTAIL_CHECK(arg.kind == Arg::Kind::String)
      << "Incorrect kind: " << arg.kind;
  if (!arg.is_append) {
...
auto Parser::ParseLongOption(llvm::StringRef unparsed_arg) -> bool {
  COCKTAIL_CHECK(unparsed_arg.startswith("--") && unparsed_arg.size() > 2)
...
auto Parser::ParseShortOptionSeq(llvm::StringRef unparsed_arg) -> bool {
  COCKTAIL_CHECK(unparsed_arg.startswith("-") && unparsed_arg.size() > 1)
...
auto Parser::FinalizeParse() -> ParseResult {
  ...
  if (appending_to_positional_arg_) {
    COCKTAIL_CHECK(static_cast<size_t>(positional_arg_index_) <
  ...
  //     COCKTAIL_CHECK(!arg_ptr->is_required)
  ...
  switch (command_->kind) {
    case Command::Kind::Invalid:
      COCKTAIL_FATAL() << "Should never have a parser with an invalid command!";
...
auto Parser::ParsePositionalSuffix(
  ...
  COCKTAIL_CHECK(!command_->positional_args.empty())
      << "Cannot do positional suffix parsing without positional arguments!";
  COCKTAIL_CHECK(!unparsed_args.empty() && unparsed_args.front() == "--")
...
auto Parser::Parse(llvm::ArrayRef<llvm::StringRef> unparsed_args)
  ...
  while (!unparsed_args.empty()) {
    ...
    if (unparsed_arg.startswith("-") && unparsed_arg.size() > 1) {
...
void CommandBuilder::AddSubcommand(
  ...
  COCKTAIL_CHECK(IsValidName(info.name))
      << "Invalid subcommand name: " << info.name;
  COCKTAIL_CHECK(subcommand_names_.insert(info.name).second)
...
auto CommandBuilder::AddArgImpl(const ArgInfo& info, Arg::Kind kind) -> Arg& {
  ...
  COCKTAIL_CHECK(arg_names_.insert(info.name).second)
...
void CommandBuilder::AddPositionalArgImpl(
  ...
  COCKTAIL_CHECK(!arg.is_help_hidden)
      << "Cannot have a help-hidden positional argument.";

  if (arg.is_required && command_.positional_args.size() > 1) {
    COCKTAIL_CHECK((*std::prev(command_.positional_args.end(), 2))->is_required)
