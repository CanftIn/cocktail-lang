...
auto Context::ConsumeAndAddCloseSymbol(Lex::Token expected_open,
  ...
  if (!open_token_kind.is_opening_symbol()) {
    ...
    COCKTAIL_DIAGNOSTIC(ExpectedCloseSymbol, Error,
...
auto Context::ConsumeChecked(Lex::TokenKind kind) -> Lex::Token {
  COCKTAIL_CHECK(PositionIs(kind))
...
auto Context::IsLexicallyValidInfixOperator() -> bool {
  COCKTAIL_CHECK(position_ != end_) << "Expected an operator token.";
...
auto Context::GetDeclarationContext() -> DeclarationContext {
  ...
  COCKTAIL_CHECK(!state_stack_.empty() &&
...
auto Context::EmitExpectedDeclarationSemi(Lex::TokenKind expected_kind)
    -> void {
  COCKTAIL_DIAGNOSTIC(ExpectedDeclarationSemi, Error,
                      "`{0}` declarations must end with a `;`.",
                      Lex::TokenKind);
...
auto Context::EmitExpectedDeclarationSemiOrDefinition(
  ...
  COCKTAIL_DIAGNOSTIC(ExpectedDeclarationSemiOrDefinition, Error,
...
auto Context::PrintForStackDump(llvm::raw_ostream& output) const -> void {
  output << "Parser stack:\n";
  for (const auto& item : llvm::enumerate(state_stack_)) {
    output << "\t" << item.index() << ".\t" << item.value().state;
    PrintTokenForStackDump(output, item.value().token);
  }
