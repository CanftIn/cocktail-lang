...
auto HandleBraceExpression(Context& context) -> void {
  ...
  COCKTAIL_CHECK(context.ConsumeAndAddLeafNodeIf(
...
static auto HandleBraceExpressionParameterError(Context& context,
  ...
  COCKTAIL_CHECK(is_type || is_value || is_unknown);
  COCKTAIL_DIAGNOSTIC(ExpectedStructLiteralField, Error, "Expected {0}{1}{2}.",
...
static auto HandleBraceExpressionParameterAfterDesignator(
  ...
  if (param_finish_state == State::BraceExpressionParameterFinishAsUnknown) {
    ...
    COCKTAIL_CHECK(finish_state.state == State::BraceExpressionFinishAsUnknown);
    if (is_type) {
