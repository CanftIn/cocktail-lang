...
class NodeNamer {
 public:
  ...
  NodeNamer(const Lex::TokenizedBuffer& tokenized_buffer,
    ...
    for (int i : llvm::seq(0, semantics_ir.functions_size())) {
    ...
    class Name {
     public:
      ...
      auto str() const -> llvm::StringRef {
        ...
        COCKTAIL_CHECK(value) << "cannot print a null name";
...
class Formatter {
 public:
  ...
  auto Format() -> void {
    ...
    for (int i : llvm::seq(0, semantics_ir_.functions_size())) {
