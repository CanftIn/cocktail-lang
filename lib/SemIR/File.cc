...
#include "Cocktail/SemIR/BuiltinKind.def"
  ...
  COCKTAIL_CHECK(nodes_.size() == BuiltinKind::ValidCount)
...
File::File(std::string filename, const File* builtins)
    ...
    : filename_(std::move(filename)),
      ...
      // Default entry for NodeBlockId::Empty.
      node_blocks_(1) {
  COCKTAIL_CHECK(builtins != nullptr);
  COCKTAIL_CHECK(builtins->cross_reference_irs_[0] == builtins)
      << "Not called with builtins!";

  // Copy builtins over.
  nodes_.reserve(BuiltinKind::ValidCount);
  static constexpr auto BuiltinIR = CrossReferenceIRId(0);
  for (const auto& item : llvm::enumerate(builtins->nodes_)) {
    // We can reuse builtin type IDs because they're special-cased values.
    nodes_.push_back(Node::CrossReference::Make(
        item.value().type_id(), BuiltinIR, SemIR::NodeId(item.index())));
  }
}
...
static auto GetTypePrecedence(NodeKind kind) -> int {
  ...
  switch (kind) {
    ...
    case NodeKind::VarStorage:
      COCKTAIL_FATAL() << "GetTypePrecedence for non-type node kind " << kind;
  }
}
...
auto GetValueRepresentation(const File& file, TypeId type_id)
  ...
  while (true) {
    ...
    switch (node.kind()) {
      ...
      case NodeKind::ValueAsReference:
      case NodeKind::VarStorage:
        COCKTAIL_FATAL() << "Type refers to non-type node " << node;
