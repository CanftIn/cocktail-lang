...
FileContext::FileContext(llvm::LLVMContext& llvm_context,
  ...
  COCKTAIL_CHECK(!semantics_ir.has_errors())
...
auto FileContext::Run() -> std::unique_ptr<llvm::Module> {
  COCKTAIL_CHECK(llvm_module_) << "Run can only be called once.";

  // Lower types.
  auto types = semantics_ir_->types();
  types_.resize_for_overwrite(types.size());
  for (const auto& item : llvm::enumerate(types)) {
    types_[item.index()] = BuildType(item.value());
  }

  // Lower function declarations.
  functions_.resize_for_overwrite(semantics_ir_->functions_size());
  for (auto i : llvm::seq(0, semantics_ir_->functions_size())) {
...
auto FileContext::BuildFunctionDeclaration(SemIR::FunctionId function_id)
  ...
  COCKTAIL_CHECK(return_rep.has_return_slot() == has_return_slot);
  ...
  switch (node.kind()) {
    ...
    case SemIR::NodeKind::StructType: {
      ...
      for (auto ref_id : refs) {
        ...
        COCKTAIL_CHECK(field_type_id.index < SemIR::BuiltinKind::ValidCount)
            << field_type_id;
    ...
    default: {
      COCKTAIL_FATAL() << "Cannot use node as type: " << node_id;
    }
  }
}
