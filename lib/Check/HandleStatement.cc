...
auto HandleReturnStatement(Context& context, Parse::Node parse_node) -> bool {
  COCKTAIL_CHECK(!context.return_scope_stack().empty());
  const auto& fn_node =
  ...
  if (context.parse_tree().node_kind(context.node_stack().PeekParseNode()) ==
    ...
    if (callable.return_type_id.is_valid()) {
      ...
      COCKTAIL_DIAGNOSTIC(ReturnStatementMissingExpression, Error,
    ...
    if (!callable.return_type_id.is_valid()) {
      ...
      COCKTAIL_DIAGNOSTIC(ReturnStatementImplicitNote, Note,
