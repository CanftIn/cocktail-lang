...
#include "Cocktail/Check/DeclarationNameStack.h"
...
auto Context::TODO(Parse::Node parse_node, std::string label) -> bool {
  COCKTAIL_DIAGNOSTIC(SemanticsTodo, Error, "Semantics TODO: `{0}`.",
                      std::string);
...
auto Context::VerifyOnFinish() -> void {
  ...
  COCKTAIL_CHECK(name_lookup_.empty()) << name_lookup_.size();
  COCKTAIL_CHECK(scope_stack_.empty()) << scope_stack_.size();
  COCKTAIL_CHECK(node_block_stack_.empty()) << node_block_stack_.size();
  COCKTAIL_CHECK(params_or_args_stack_.empty()) << params_or_args_stack_.size();
}

auto Context::AddNode(SemIR::Node node) -> SemIR::NodeId {
  auto node_id = node_block_stack_.AddNode(node);
  COCKTAIL_VLOG() << "AddNode: " << node << "\n";
  return node_id;
}
...
auto Context::DiagnoseDuplicateName(Parse::Node parse_node,
  ...
  COCKTAIL_DIAGNOSTIC(NameDeclarationDuplicate, Error,
                      "Duplicate name being declared in the same scope.");
  COCKTAIL_DIAGNOSTIC(NameDeclarationPrevious, Note,
...
auto Context::DiagnoseNameNotFound(Parse::Node parse_node,
  ...
  COCKTAIL_DIAGNOSTIC(NameNotFound, Error, "Name `{0}` not found.",
                      llvm::StringRef);
...
auto Context::LookupName(Parse::Node parse_node, SemIR::StringId name_id,
  ...
  if (scope_id == SemIR::NameScopeId::Invalid) {
    ...
    COCKTAIL_CHECK(!it->second.empty())
...
auto Context::AddConvergenceBlockAndPush(Parse::Node parse_node, int num_blocks)
    -> void {
  COCKTAIL_CHECK(num_blocks >= 2) << "no convergence";

  SemIR::NodeBlockId new_block_id = SemIR::NodeBlockId::Unreachable;
  for ([[maybe_unused]] auto _ : llvm::seq(0, num_blocks)) {
...
auto Context::AddConvergenceBlockWithArgAndPush(
    ...
    -> SemIR::NodeId {
  COCKTAIL_CHECK(block_args.size() >= 2) << "no convergence";
...
auto Context::AddCurrentCodeBlockToFunction() -> void {
  COCKTAIL_CHECK(!node_block_stack().empty()) << "no current code block";
  COCKTAIL_CHECK(!return_scope_stack().empty()) << "no current function";
...
auto Context::CanonicalizeTypeImpl(
  ...
  COCKTAIL_CHECK(canonical_types_.insert({node_id, type_id}).second);
...
static auto ProfileType(Context& semantics_context, SemIR::Node node,
  ...
  switch (node.kind()) {
    ...
    case SemIR::NodeKind::TupleType:
      ...
      break;
    default:
      COCKTAIL_FATAL() << "Unexpected type node " << node;
  }
}
