...
static auto FindReturnSlotForInitializer(SemIR::File& semantics_ir,
  ...
  switch (init.kind()) {
    default:
      COCKTAIL_FATAL() << "Initialization from unexpected node " << init;
...
static auto MarkInitializerFor(SemIR::File& semantics_ir, SemIR::NodeId init_id,
  ...
  if (return_slot_id.is_valid()) {
    ...
    COCKTAIL_CHECK(semantics_ir.GetNode(return_slot_id).kind() ==
...
static auto ConvertTupleToArray(Context& context, SemIR::Node tuple_type,
  ...
  if (tuple_elem_types.size() != array_bound) {
    COCKTAIL_DIAGNOSTIC(
        ...
        uint64_t, size_t);
    COCKTAIL_DIAGNOSTIC(ArrayInitFromExpressionArgCountMismatch, Error,
  ...
  for (auto item : llvm::enumerate(tuple_elem_types)) {
    auto i = item.index();
    auto src_type_id = item.value();
...
static auto ConvertTupleToTuple(Context& context, SemIR::Node src_type,
  ...
  if (src_elem_types.size() != dest_elem_types.size()) {
    COCKTAIL_DIAGNOSTIC(TupleInitElementCountMismatch, Error,
  ...
  CopyOnWriteBlock new_block(semantics_ir,
                             ...
                             src_elem_types.size());
  for (size_t i = 0; i < src_elem_types.size() && i < dest_elem_types.size();
       ++i) {
    auto src_type_id = src_elem_types[i];
    auto dest_type_id = dest_elem_types[i];
...
static auto ConvertStructToStruct(Context& context, SemIR::Node src_type,
  ...
  for (size_t i = 0; i < src_elem_fields.size() && i < dest_elem_fields.size();
       ++i) {
    auto src_field_id = src_elem_fields[i];
    auto dest_field_id = dest_elem_fields[i];
...
auto Convert(Context& context, Parse::Node parse_node, SemIR::NodeId expr_id,
  ...
  if (SemIR::GetExpressionCategory(semantics_ir, expr_id) ==
    ...
    COCKTAIL_DIAGNOSTIC(UseOfNonExpressionAsValue, Error,
  ...
  if (expr.type_id() != target.type_id) {
    COCKTAIL_DIAGNOSTIC(ImplicitAsConversionFailure, Error,
  ...
  switch (SemIR::GetExpressionCategory(semantics_ir, expr_id)) {
    ...
    case SemIR::ExpressionCategory::Mixed:
      COCKTAIL_FATAL() << "Unexpected expression " << expr
...
auto ConvertCallArgs(Context& context, Parse::Node call_parse_node,
  ...
  if (has_return_slot) {
    ...
    COCKTAIL_CHECK(!arg_refs.empty()) << "missing return slot";
    arg_refs = arg_refs.drop_back();
  }

  // If sizes mismatch, fail early.
  if (arg_refs.size() != param_refs.size()) {
    COCKTAIL_DIAGNOSTIC(CallArgCountMismatch, Error,
                        "{0} argument(s) passed to function expecting "
                        "{1} argument(s).",
                        int, int);
    COCKTAIL_DIAGNOSTIC(InCallToFunction, Note,
  ...
  for (size_t i = 0; i < arg_refs.size() && i < param_refs.size(); ++i) {
    diag_param_index = i;

    auto value_id = arg_refs[i];
    auto param_ref = param_refs[i];
